...
module.exports = function(FlexFundsDB) {
  ...
  this.findOptions = function(table, selectType) {
    var query = '';
    if (table == 'ib_activity'){
      query = 'select distinct(concat(series_number, " - ", account_id, " - ", bloomberg_name)) label, account_number value from '
        + table +  ' t0, series_product_information t1 where t0.account_id = t1.account_number order by value;';
    }
    else if (table == 'pershing_trades'){
      query = 'select distinct(concat(series_number, " - ", account, " - ", bloomberg_name)) label, account_number value from '
        + table +  ' t0, series_product_information t1 where t0.account = t1.account_number order by value;';
    }
      ...
      query = 'select distinct(concat(t0.series_number, " - ", isin, " - ", six_name)) label, t0.series_number value from '
        + table +  ' t0, series_names t1 where t0.series_number = t1.series_number;';
    }
    else if (selectType == 'isin'){
      query = 'select distinct(concat(series_number, " - ", t0.isin, " - ", six_name)) label, t0.isin value from '
          + table +  ' t0, series_names t1 where t0.isin = t1.isin;';
    }
    else {
